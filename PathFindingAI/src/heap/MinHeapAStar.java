...
public class MinHeapAStar {
    ...
    public MinHeapAStar(ArrayList<Pixel> items) {
    ...
    public void insert(Pixel item) {
        ...
        while (parent != i && list.get(i).getCost() < list.get(parent).getCost()) {
...
//    }
    ...
    private void minHeapify(int i) {
        ...
        if (left <= list.size() - 1 && list.get(left).getCost() < list.get(i).getCost()) {
            smallest = left;
        } else {
            smallest = i;
        }

        if (right <= list.size() - 1 && list.get(right).getCost() < list.get(smallest).getCost()) {
            smallest = right;
        }
